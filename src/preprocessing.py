...
def detect_trim(time, y_values, step, stride, baseline_windows=10, factor=2.0, required_hits=3):
    """
    Detect trim start by comparing average of first baseline_windows
    against subsequent windows.

    Parameters
    ----------
    time : np.ndarray
        Array of elapsed seconds.
    y_values : np.ndarray
    ...
    baseline_windows : int
        Number of windows to use for baseline average.
    factor : float
        Threshold multiplier for activity detection.
    required_hits : int
        Number of consecutive windows exceeding baseline before confirming trim.
    """
    # Compute baseline from first N windows
    baseline_mins, baseline_maxs = [], []
    for i in range(baseline_windows):
        ...
        baseline_mins.append(window_y.min())
        baseline_maxs.append(window_y.max())

    baseline_min = np.mean(baseline_mins) if baseline_mins else np.min(y_values)
    baseline_max = np.mean(baseline_maxs) if baseline_maxs else np.max(y_values)

    cutoff_times = []
    consecutive_hits = 0

    # Start scanning after baseline
    for start in range(baseline_windows * step, len(y_values) - step + 1, stride):
        end = start + step
        window_y = y_values[start:end]
        if len(window_y) == 0:
            continue

        curr_min, curr_max = window_y.min(), window_y.max()

        # Compare against baseline
        if (curr_max > factor * baseline_max): #or (curr_min < 0.5 * baseline_min):
            consecutive_hits += 1
    ...
    return cutoff_times[0] if cutoff_times else time[0]

def detect_trim_bounds_slope(
    time, y_values, step, stride,
    slope_thr_start=0.05,   # minimum avg slope to declare start
    slope_thr_end=-0.03,    # negative slope threshold for ending
    required_hits=3,
    smooth=5,
    debug=False
):
    ...
    # Compute slope (difference over time)
    slopes = np.gradient(rms_vals_smooth, rms_times)

    # --- Detect start (sustained positive slope) ---
    consec = 0
    trim_start = rms_times[0]
    for i in range(len(slopes)):
        if slopes[i] > slope_thr_start:
            consec += 1
            if consec >= required_hits:
                trim_start = rms_times[i - required_hits + 1]
                break
        else:
            consec = 0

    # --- Detect end (sustained negative slope) ---
    consec = 0
    trim_end = rms_times[-1]
    for i in range(len(slopes) - 1, -1, -1):
        if slopes[i] < slope_thr_end:
            consec += 1
            if consec >= required_hits:
                trim_end = rms_times[i + required_hits - 1]
                break
        else:
            consec = 0
    ...
    if debug:
        ...
        plt.plot(rms_times, slopes, label="Slope", color="orange", alpha=0.6)
        ...
        plt.axhline(slope_thr_start, color="red", linestyle="--", label="Slope thr start")
        plt.axhline(slope_thr_end, color="purple", linestyle="--", label="Slope thr end")
        plt.xlabel("Time [s]"); plt.ylabel("RMS / slope"); plt.legend()
        plt.title("Slope-based trim detection")
        plt.show()
